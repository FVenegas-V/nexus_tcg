...
class _ReputationDashboardScreenState extends State<ReputationDashboardScreen>
  ...
  void _loadInitialData() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Asegurar que RatingsService tenga el token actualizado
      AuthService.getAccessToken().then((token) {
        if (token != null) {
          RatingsService.instance.updateAuthToken(token);
          debugPrint('🔑 Token actualizado en RatingsService para dashboard');
        }
      });

      debugPrint('🔄 Cargando datos de reputación para usuario $currentUserId');
  ...
  Widget _buildBody() {
    return Consumer3<ReputationProvider, RatingsProvider, AuthProvider>(
      builder:
          (context, reputationProvider, ratingsProvider, authProvider, child) {
            ...
            // Obtener ID del usuario actual desde AuthProvider
            final currentUserId = authProvider.user?['id'] as int?;
            if (currentUserId == null) {
              debugPrint('❌ No hay usuario autenticado en _buildBody');
              return _buildErrorState('Usuario no autenticado');
            }
  ...
  Widget _buildRecentRatingsSection(RatingsProvider ratingsProvider) {
    ...
    if (currentUserId == null) {
      return Container(); // No mostrar nada si no hay usuario
    }
